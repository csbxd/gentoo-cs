...
# Flutter's frontend server and gen_snapshot exchange Kernel binaries and
# validate the SDK hash embedded in them. Keep the system SDK exactly aligned
# with the Dart release used by the pinned Flutter engine.
RDEPEND="
	${DEPEND}
...
ARM64 system Dart SDK from dev-lang/dart-3.13.1. The version must match the
pinned Flutter engine because its AOT tools validate the Dart SDK hash.

Enable and sync the Bentoo repository to provide this Dart package. If Bentoo
has removed the matching patch release, retain it locally rather than using a
newer Dart SDK with this Flutter engine.
